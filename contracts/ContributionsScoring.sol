...
contract ContributionScoring {
    ...
    function calculateRIS(uint256 stars, uint256 forks, uint256 contributors) public pure returns (uint256) {
    ...
    function calculateCQS(uint256 commitsByUser, uint256 totalAdditions) public pure returns (uint256) {
    ...
    function calculatePPB(uint256 productionCommits) public pure returns (uint256) {
        // PPB = productionCommits * 5
        return productionCommits * 5;
    }

    // Calculate the final score for a single repo
    function calculateRepoScore(RepoMetrics memory m) public pure returns (uint256) {
